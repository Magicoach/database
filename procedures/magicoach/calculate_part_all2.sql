DELIMITER $$
CREATE PROCEDURE `calculate_part_all2`( IN new_part int) 
...
DROP TABLE IF EXISTS tmp_table_21;
DROP TABLE IF EXISTS tmp_table_31;
DROP TABLE IF EXISTS tmp_table_32;
...
-- >>>>>>>> CHECK "MY GOAL"
SET @goal_check =  IF((SELECT SUM(delta_t) FROM `tmp_table` WHERE X_temp >= 0 AND Y_temp >= 0 AND X_temp <= 1 AND Y_temp <= 0.7)>
        (SELECT SUM(delta_t) FROM `tmp_table` WHERE X_temp >= 0 AND Y_temp >= 0.3 AND X_temp <= 1 AND Y_temp <= 1),1,-1);
...
-- >>>>>>>> STATS
  INSERT INTO `12_physical_stats` (ID_Participation, max_speed, avg_speed, distance, percent_running, time_played, sprints)
    ...
    WHERE `21_participations`.ID_Participation = @new_part AND X_percent >= 0 AND Y_percent >= 0 AND X_percent <= 1 AND Y_percent <= 1 AND speed <= (SELECT Max_allowed_speed FROM `1_algorithm_constants`)
...
-- >>>>>>>> AREAS 
  INSERT INTO `12_areas` (id_participation, defense, middle, attack)
    ...
    FROM `1_analyzed_data`
    WHERE ID_Participation = @new_part AND X_percent >= 0 AND Y_percent >= 0 AND X_percent <= 1 AND Y_percent <= 1 AND speed <= (SELECT Max_allowed_speed FROM `1_algorithm_constants`)
...
WHERE ID_Participation = @new_part;
  ...
  INSERT `12_map` (ID_Participation, X_percent, Y_percent, Map_value)
    ...
    WHERE `1_analyzed_data`.ID_Participation = @new_part AND `1_analyzed_data`.X_percent >= 0 AND `1_analyzed_data`.Y_percent >= 0 AND `1_analyzed_data`.X_percent <= 1 AND `1_analyzed_data`.Y_percent <= 1 AND `1_analyzed_data`.speed <= (SELECT Max_allowed_speed FROM `1_algorithm_constants`)
...
-- >>>>>>>> DIRECTIONS
   INSERT `12_directions` (id_participation, up, up_right, `right`, down_right, down, down_left, `left`, up_left)
    ...
    @h7 :=  SUM(IF(direction >= 247.5 AND direction < 292.5, (DISTANCE), 0)) / 1000 * 0.5 AS `left`,
     ...
     FROM `1_analyzed_data`
     WHERE `1_analyzed_data`.ID_Participation = @new_part AND X_percent >= 0 AND Y_percent >= 0 AND `1_analyzed_data`.X_percent <= 1 AND `1_analyzed_data`.Y_percent <= 1 AND `1_analyzed_data`.speed > (SELECT Sprint_treshold FROM `1_algorithm_constants`)  AND `1_analyzed_data`.speed <= (SELECT Max_allowed_speed FROM `1_algorithm_constants`)
