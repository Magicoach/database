DELIMITER $$
CREATE PROCEDURE `calculate_part_all4`(IN new_part int) 
...
*/
  ...
  INSERT INTO `12_physical_stats` (ID_Participation, max_speed, avg_speed, distance, percent_running, time_played, sprints)
    SELECT
      ...
      FROM `1_algorithm_constants`), delta_t, 0)) / SUM(delta_t) AS percent_running,
  ...
  INSERT INTO `12_areas` (id_participation, defense, middle, attack)
    SELECT
      ...
      @defense_1 := SUM(IF(`1_analyzed_data`.Y_percent <= 0.33, (`1_analyzed_data`.distance), 0)) / 1000 AS defense,
      @middle_1 := SUM(IF(`1_analyzed_data`.Y_percent > 0.33 AND `1_analyzed_data`.Y_percent < 0.66, (`1_analyzed_data`.distance), 0)) / 1000 AS middle,
      @attack_1 := SUM(IF(`1_analyzed_data`.Y_percent >= 0.66, (`1_analyzed_data`.distance), 0)) / 1000 AS attack
    FROM `1_analyzed_data`
    WHERE ID_Participation = @new_part AND `1_analyzed_data`.X_percent >= 0 AND `1_analyzed_data`.Y_percent >= 0 AND
      `1_analyzed_data`.X_percent <= 1 AND `1_analyzed_data`.Y_percent <= 1 AND `1_analyzed_data`.speed <= (SELECT
      Max_allowed_speed
    FROM `1_algorithm_constants`)
 ...
 --   GROUP BY ID_Participation;
  ...
  INSERT `12_magicpoints` (ID_Participation, magicpoints, tactic_zones, field_ocupation, game_style)
    SELECT
      ...
      @f3 := (SELECT COUNT(`12_map`.Map_value) FROM `12_map` WHERE `12_map`.ID_Participation = @new_part) AS field_ocupation,
...
--  ON DUPLICATE KEY UPDATE Total_goals = @f1, Total_points = @f2;
  ...
  -- >>>>>>>> REARANGE DATA
  UPDATE `21_participations` SET `21_participations`.status = 1 WHERE `21_participations`.ID_Participation = @new_part;

  DELETE FROM `1_analyzed_data` WHERE `1_analyzed_data`.ID_Participation = @new_part;
