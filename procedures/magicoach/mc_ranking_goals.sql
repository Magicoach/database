DELIMITER $$
CREATE PROCEDURE `mc_ranking_goals`(IN id_user int, IN date_start datetime, IN date_end datetime, IN order_by varchar(255)) 
 SQL SECURITY DEFINER  CONTAINS SQL BEGIN

  -- CALL mc_ranking_goals(10350, '2014.01.01 10:00:00', '2017.01.01 10:00:00', 'max');
 ...
 IF (id_user=-100) THEN set @activestatus = 2; ELSE set @activestatus = 1; END IF;
    ...
    CREATE TEMPORARY TABLE IF NOT EXISTS ranking AS (SELECT
        ...
        u.username,
        u.image,
        SUM(p.my_goals) AS `sum`,
        ROUND(AVG(p.my_goals), 1) AS `avg`,
        MAX(p.my_goals) AS `max`,
      ...
      GROUP BY u.ID
      ORDER BY CASE WHEN order_by = 'sum' THEN SUM(p.my_goals) WHEN order_by = 'avg' THEN AVG(p.my_goals) WHEN order_by = 'max' THEN MAX(p.my_goals) END DESC, u.ID ASC);
