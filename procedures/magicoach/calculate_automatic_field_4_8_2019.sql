DELIMITER $$
CREATE PROCEDURE `calculate_automatic_field_4_8_2019`(IN ID_part int,OUT id_new_fied INT) 
 SQL SECURITY DEFINER  CONTAINS SQL BEGIN

  SET @id_participation = ID_part; -- 5287;
...
SET @new_width = (SELECT a.width -- , AVG(a.INDEX_finder)
  FROM (
 SELECT
    daf.width,
    daf.height,
  ...
  ORDER BY AVG(a.INDEX_finder) DESC
  LIMIT 1);

 -- FIND NEW FIELD HEIGHT  
SET @new_height = (SELECT a.height -- , AVG(a.INDEX_finder)
  FROM (
 SELECT
    daf.width,
    daf.height,
...
SET @new_rotation = (SELECT 90-a.rotation -- , AVG(a.INDEX_finder)
  FROM (
 SELECT
    daf.width,
    daf.height,
...
SET @angle_to_corner =  180/PI()*ATAN(@new_height/@new_width);
...
SET @count_gps = (SELECT COUNT(*) FROM `0_positions` WHERE ID_Participation=@id_participation);
IF @count_gps < 300 THEN SET @field_name = "Tiny GPS Test";
  ELSEIF @count_gps BETWEEN 300 AND 900 THEN SET @field_name = "Test Match";
  ELSE SET @field_name = "Auto Calibrated";
END IF; 
...
INSERT INTO `21_field_coordinates` (Lat_esq_baliza1, Long_esq_baliza1, Lat_dir_baliza1, Long_dir_baliza1, Lat_esq_baliza2,  Long_esq_baliza2, typical_team_size, Field_name, Lat_avg, Long_avg, date_created)
  VALUES (@Lat_esq_baliza1, @Long_esq_baliza1, @Lat_dir_baliza1, @Long_dir_baliza1, @Lat_esq_baliza2, @Long_esq_baliza2, 7, @field_name, @center_latitude, @center_longitude, NOW());

SET id_new_fied = (SELECT LAST_INSERT_ID());
