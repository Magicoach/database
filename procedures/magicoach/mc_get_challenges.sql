DELIMITER $$
CREATE PROCEDURE `mc_get_challenges`(IN id_user int) 
 SQL SECURITY DEFINER  CONTAINS SQL BEGIN
  ...
  START TRANSACTION;
    ...
    IF is_active_user(id_user) THEN
      ...
      FROM `2_challenges` c
        JOIN `3_users` u  ON u.ID = id_user
      WHERE c.visible = 1 AND (c.id_challenger = id_user OR c.id_challenged = id_user)
