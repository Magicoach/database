DELIMITER $$
CREATE PROCEDURE `mc_ranking_magicpoints`(IN id_user int, IN date_start datetime, IN date_end datetime, IN order_by varchar(255)) 
 SQL SECURITY DEFINER  CONTAINS SQL BEGIN

  -- CALL mc_ranking_magicpoints_copy(10350, '2016.01.01 10:00:00', '2017.01.01 10:00:00', 'max');
  ...
  IF (id_user=-100) THEN set @activestatus = 2; ELSE set @activestatus = 1; END IF;
    ...
    CREATE TEMPORARY TABLE IF NOT EXISTS friends AS (
      SELECT
        u2.ID AS id_friend
      FROM `3_users_invitations` i
        JOIN `3_users` u1 ON (u1.ID = i.id_sender OR u1.ID = i.id_receiver)
        JOIN `3_users` u2 ON (u2.ID = i.id_receiver OR u2.ID = i.id_sender) AND u1.ID <> u2.ID
      WHERE u1.ID = id_user
    ...
    CREATE TEMPORARY TABLE IF NOT EXISTS ranking AS (SELECT
        ...
        u.username,
        u.image,
        SUM(m.magicpoints) AS `sum`,
        ROUND(AVG(m.magicpoints), 1) AS `avg`,
        MAX(m.magicpoints) AS `max`,
      ...
      FROM `21_participations` p
        JOIN `12_magicpoints` m ON p.ID_Participation = m.ID_Participation
      ...
      GROUP BY u.ID
      ORDER BY CASE WHEN order_by = 'sum' THEN SUM(m.magicpoints) WHEN order_by = 'avg' THEN AVG(m.magicpoints) WHEN order_by = 'max' THEN MAX(m.magicpoints) END DESC, u.ID ASC);
    ...
    (SELECT
      ...
      JOIN `21_field_coordinates` fc ON fc.ID_Field = p.ID_Field
      WHERE u.ID = id_user
      AND fc.ID_Field > 0 AND u.active = @activestatus  AND IF (id_user=-100, u.ID<-99, u.ID > 0)
      AND m.magicpoints > 0
      AND p.time_start >= date_start
      AND p.time_end <= date_end
      GROUP BY u.ID)
